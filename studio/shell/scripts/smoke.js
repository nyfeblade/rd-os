...
assert.match(html, /id="seat-providers"/);
assert.match(html, /id="cutover-error"/);
assert.match(html, /id="shell-toast"/);
assert.match(html, /seat-connect\.js/);
assert.match(html, /<h1>Connect<\/h1>/);
assert.match(html, /id="cta-github">GitHub</);
assert.match(html, /id="cta-import">Import team</);
assert.match(html, /id="cta-seat"[^>]*>Connect</);
assert.doesNotMatch(html, /id="cta-seat">Seat</);
assert.doesNotMatch(html, /Connect GitHub and an agent/);
...
assert.match(html, /id="bind-status"/);
assert.match(html, /id="seat-list"/);
assert.match(html, /aria-label="Team"/);
assert.doesNotMatch(html, /id="seat-list" hidden/);
...
assert.match(html, /id="cta-import"/);
...
assert.match(css, /grid-template-columns:\s*1fr 300px/);
assert.match(css, /grid-template-columns:\s*1fr 300px 280px/);
...
assert.match(css, /pre\.payload/);
assert.match(css, /pre\.diff/);
...
assert.doesNotMatch(js, /attention\.empty\.json/);
assert.doesNotMatch(js, /attention\.(human|dump)/);
assert.match(js, /loadCatalog/);
assert.match(js, /FALLBACK_P0/);
assert.match(js, /mode: "eng"/);
assert.doesNotMatch(js, /applyLiveDemo/);
assert.match(js, /githubConnected/);
assert.match(js, /auth\.start/);
assert.doesNotMatch(js, /viewCold|viewLive|view-cold|view-live/);
assert.match(js, /needs_auth/);
assert.match(js, /applyLiveState/);
...
assert.doesNotMatch(js, /pendingHitl/);
assert.doesNotMatch(js, /Deploy to production/);
assert.match(js, /state\.gates|gates:/);
assert.match(js, /needsHitlCard/);
assert.match(js, /seatsInvoke\("connect"/);
assert.match(js, /openAddSeat/);
assert.match(js, /showSeatError/);
assert.match(js, /applySeatConnect/);
assert.match(js, /CONNECT_ACK|This seat works in Studio only while connected/);
assert.match(read("renderer/seat-connect.js"), /This seat works in Studio only while connected/);
assert.match(read("renderer/seat-connect.js"), /claude|grok|cursor|codex|gemini|chatgpt/);
assert.doesNotMatch(js, /seat\.presence\s*=\s*"online"/);
assert.match(js, /chatInvoke\("bind"/);
assert.match(js, /chatInvoke\("send"/);
assert.match(js, /studioShell\.chat/);
assert.doesNotMatch(js, /Studio\.panes\.pushLocal\(seat\.id/);
assert.match(chat, /HITL pending/);
...
assert.match(chat, /id: "elon"/);
assert.match(chat, /Import team/);
assert.match(chat, /Team/);
assert.doesNotMatch(chat, /Talk to agents/);
assert.match(chat, /in-studio-only/);
assert.match(chat, /seatList.hidden = false/);
assert.match(chat, /Connect/);
...
assert.match(chat, /composer is the draft/);
assert.match(chat, /engineMessages|engine\.messages/);
assert.match(chat, /Slack · sign in/);
assert.doesNotMatch(chat, /person on/);
assert.match(chat, /Eng Lead/);
assert.doesNotMatch(chat, /VERIFIED/);
assert.doesNotMatch(chat, /Proof VERIFIED/);

assert.match(board, /Needs you/);
assert.match(board, /HITL/);
assert.match(board, /Approve/);
assert.match(board, /payload/);
assert.match(board, /destination/);
assert.match(board, /actor/);
assert.match(board, /needsHitlCard/);
assert.match(board, /Merge gate/);
assert.match(board, /instrumentFor/);
assert.match(board, /Diff/);
assert.doesNotMatch(board, /Agent map/);
assert.doesNotMatch(board, /Approve send/);
assert.doesNotMatch(board, /HITL · high-risk outbound/);
...
assert.match(tray, /sign in/);
assert.doesNotMatch(tray, /state.view === "cold"/);
...
assert.match(presence, /shouldShowPresence/);
assert.match(presence, /in-studio-only/);
assert.match(presence, /presenceBtn.hidden = !show/);

assert.match(tokens, /--bg:\s*#fff/);
assert.match(tokens, /--accent:\s*#0a0a0a/);
assert.match(tokens, /--ink:\s*#0a0a0a/);
assert.match(tokens, /--line:\s*#e2e8f0/);
assert.match(tokens, /--slate:\s*#334155/);
...
assert.match(main, /studio:chat.bind/);
assert.match(main, /studio:chat.send/);
assert.match(main, /createChatSession/);
assert.match(main, /createStudioLive/);
assert.match(main, /createSeatsSession/);
assert.match(main, /studio:seats.connect/);
assert.match(main, /studio:auth.start/);
...
assert.match(readme, /chat-engine/);
assert.match(readme, /prove-chat|npm test/);
...
assert.match(presence, /in-studio/);
...
assert.match(read("design/CONNECTORS-TWO-WAY.md"), /bidirectional|HITL/);
assert.ok(fs.existsSync(path.join(root, "design", "HITL.md")));
assert.ok(fs.existsSync(path.join(root, "lib", "hitl.js")));
assert.equal(hitl.egressRisk("deploy"), "high");
assert.equal(hitl.egressRisk("issue_comment"), "low");
assert.equal(hitl.egressRisk("message"), "low");
assert.equal(hitl.needsHitlCard("public_post"), true);
assert.equal(hitl.needsHitlCard("mention"), false);
assert.match(read("design/HITL.md"), /Pending-approval/);
...
assert.match(read("scripts/preview.js"), /\/chat\/bind/);
assert.match(read("scripts/preview.js"), /\/chat\/send/);
assert.match(read("lib/chat-bridge.js"), /createChatEngine/);
assert.match(read("lib/chat-bridge.js"), /memory\.write/);
assert.doesNotMatch(read("lib/chat-bridge.js"), /function resolveGitRoot/);
assert.ok(fs.existsSync(path.join(root, "scripts", "prove-chat.js")));
assert.ok(fs.existsSync(path.join(root, "scripts", "prove-seat-connect.js")));
assert.ok(fs.existsSync(path.join(root, "lib", "seats-bridge.js")));
assert.ok(fs.existsSync(path.join(root, "lib", "seat-connect.js")));
assert.match(read("lib/seats-bridge.js"), /createStudioSeats/);
assert.match(read("lib/seats-bridge.js"), /mcp_attach/);
assert.doesNotMatch(read("lib/seats-bridge.js").slice(
  read("lib/seats-bridge.js").indexOf("function connect("),
  read("lib/seats-bridge.js").indexOf("function list("),
), /MISSING_SECRET|API_KEY/);
assert.match(read("lib/seat-connect.js"), /runSeatConnectClick/);
assert.match(read("electron/preload.js"), /studio:seats.connect/);
assert.match(read("electron/preload.js"), /studio:auth.start/);
assert.match(read("scripts/preview.js"), /\/seats\/connect/);
assert.match(read("scripts/preview.js"), /runSeatConnectClick/);
assert.match(read("scripts/preview.js"), /createStudioLive/);
assert.ok(fs.existsSync(path.join(root, "scripts", "prove-live.js")));
assert.match(read("package.json"), /prove-live\.js/);
