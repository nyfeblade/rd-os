...
**SoT for the connectors tray.** Researched from official vendor docs (fetched 2026-09-18). Hosted MCP URLs below appear on those pages. A cell marked **UNVERIFIED** means no official page confirmed the endpoint; do not invent one.
...
| Grok / xAI | P0 | API | [docs.x.ai overview](https://docs.x.ai/overview) · REST [api.x.ai/docs](https://api.x.ai/docs/) | Bearer `XAI_API_KEY` from the xAI console. Base `https://api.x.ai/v1`. | Seat: Grok / Grok Build complete. **No official xAI MCP found** — do not invent one. |
...
| PagerDuty | P2 | MCP | [PagerDuty MCP (support)](https://support.pagerduty.com/main/docs/pagerduty-mcp-server) · [remote setup](https://pagerduty.github.io/pagerduty-mcp-server/docs/remote-server/setup) · hosted `https://mcp.pagerduty.com/mcp` · EU `https://mcp.eu.pagerduty.com/mcp` | OAuth or User API token (`Token token=`). Local: `uvx pagerduty-mcp` (read-only default). | Incidents / on-call next to Sentry and Datadog. |
...
| dbt | P2 | MCP | [About dbt MCP](https://docs.getdbt.com/docs/dbt-ai/about-mcp) · [remote setup](https://docs.getdbt.com/docs/dbt-ai/setup-remote-mcp) · [dbt-labs/dbt-mcp](https://github.com/dbt-labs/dbt-mcp) | Remote: OAuth or `Authorization: Token\|Bearer <PAT>` + `x-dbt-prod-environment-id`. URL from Account settings, or `https://<dbt-host>/api/ai/v1/mcp`. Local: `uvx dbt-mcp`. | Models / Semantic Layer / job runs for analytics eng. |
...
| npm registry | P2 | API | [npm Public Registry API](https://github.com/npm/registry/blob/main/docs/REGISTRY-API.md) · metadata `https://registry.npmjs.org/<pkg>` | Read: none. Publish: npm token. | Resolve versions / packuments before adding a dependency. |
...
| crates.io | P2 | API | [crates.io data access](https://crates.io/data-access) · Cargo [registry web API](https://doc.rust-lang.org/cargo/reference/registry-web-api.html) | Read: none (rate limits / UA required per data-access). Publish: API token. | Crate search / owners for Rust deps. |
...
- Grafana and Snyk official MCPs are **local** (stdio / CLI), not a single public `https://mcp.*` host.
