...
`attention.dump` returns P0 and HARD LAW in one object (see `ARCHITECTURE.md`). Agents may not strip `hard_law`. Humans steer from the Waiting UI (a view of the dump) or `rdos steer.gate --actor human`.
